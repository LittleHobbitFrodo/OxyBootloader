...
impl FrameBuffer {
    ...
    /// Removes all printed elements from the framebuffer
    /// - Leaves the screen blank
    pub fn clear(&mut self) {
        unsafe {
            self.pointer.write_bytes(0, (self.w * self.h) as usize);
        }
    }
    ...
    pub fn render(&mut self, c: u8) {
        match c {
            ...
            _ => {
                let fnt = match FONT.as_ref().get_char(c) {
                    Some(f) => f,
                    None => &ERR_CHAR,
                };
                let fb = self.pointer;
                let arr = unsafe {fb.cast::<u32>().add((self.line * self.w as usize * (FONT_BITS + SPACE_BETWEEN_LINES as usize)) + self.row * FONT_BITS)};

                for i in 0..FONT_BITS {
                    for ii in 0..FONT_BITS {
                        unsafe {arr.add((i as usize * self.w as usize) + (FONT_BITS - ii as usize)).write(self.color * ((fnt[i] as u32 >> ii as u32) & 1))};
                    }
                }
                self.row += 1;
                if self.row >= self.w as usize {
                    self.endl();
                }
            }
        }
...
/// Font yoinked from my other project
...
const GET_CHAR_END: usize = FONT_SIZE + PLACE_SUB;
pub struct Font([FontChar; FONT_SIZE]);

pub const ERR_CHAR: FontChar = [0b10111011, 0b10111011, 0b11010111, 0b11101111, 0b11010111, 0b11010111, 0b10111011, 0b10111011];
...
impl Font {
    ...
    pub fn get_char(&self, i: u8) -> Option<&FontChar> {
        match i as usize {
            PLACE_SUB..GET_CHAR_END => Some(&self.0[i as usize - PLACE_SUB as usize]),
            _ => {
                None
            }
        }
